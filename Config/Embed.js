...
class ODGEmbed {
	...
	static async main(data, title, description, fields) {
		const user = await new data.client.resolvers(data.client).member(data.userId, data.guild);
		...
		if (fields) {
			Array.isArray(fields) ? embed.setFields(fields) : embed.addFields([fields]);
		}
	...
	static async latency(data, title, description, fields) {
		...
		if (fields) {
			Array.isArray(fields) ? embed.setFields(fields) : embed.setFields([fields]);
		}
	...
	static async error(name, message) {
		const embed = new EmbedBuilder()
			...
			.setDescription(`**${seekErrorTitle(name)}**\n\`\`\`\n${message}\n\`\`\``)
